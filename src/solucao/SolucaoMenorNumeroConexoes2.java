...
public class SolucaoMenorNumeroConexoes2 implements Solucao {
	private String nome = "Explosão por menor número de conexões 2";
	private String abreviacao;
	private String descricao = "";
	private String critDesempate; /*MenorFitness ou MaiorFitness*/
	
	public SolucaoMenorNumeroConexoes2(String critDesempate){
	...
	public Resultado[] gerarSolucao(Vertice[] conjuntoVertices, int[] medianasDesejadas, DistribuidorDeMedianasIniciais distribuidorDeMedianasIniciais) {
		...
		while(PM.getNumMedianas() > medianasDesejadas[0]){ //Itera enquanto for maior que a menor mediana desejada (Ex.: 5)
			...
			Mediana medianaPerfeitaEntreMedianas = pegarMedianaPerfeitaEntreMedianas(medianaEscolhida, medianaMaisProxima); 
			
			PM.removerMediana(medianaEscolhida);
			PM.removerMediana(medianaMaisProxima);
			
			PM.addMediana(medianaPerfeitaEntreMedianas);
	 ...
	 * @return
	 */
	private Mediana pegarMedianaPerfeitaEntreMedianas(Mediana mediana1, Mediana mediana2) {
		Vertice verticeDaMedianaPerfeita = null;
		double menorFitness = 0;
		
		ArrayList<Vertice> conjuntoVertices = new ArrayList<Vertice>();
		
		conjuntoVertices.addAll(Arrays.asList(mediana1.getVertices()));
		conjuntoVertices.addAll(Arrays.asList(mediana2.getVertices()));
